...
def screenshot_norm_region(region: Iterable[float]) -> tuple[Image.Image, tuple[int, int, int, int]]:
    import pyautogui
...
def locate_text_on_screen(
    ...
    *,
    ...
    image, region_abs = screenshot_norm_region(region_norm)
    found = find_text_span(image, targets, lang=lang, threshold=threshold, psm=psm)
    if found:
